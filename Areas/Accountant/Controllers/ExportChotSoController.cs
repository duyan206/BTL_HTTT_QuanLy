    ...
    public class ExportChotSoController : Controller
    {
        ...
        public async Task<IActionResult> XuatBaoCaoThang(int month, int year)
        {
            ...
            var phieuDeXuats = await _context.PhieuDeXuats
                ...
                .Where(p => p.NgayTao != null && p.NgayTao.Value.Month == month && p.NgayTao.Value.Year == year)
                .ToListAsync();
            ...
            foreach(var p in phieuDeXuats) {
                ...
                ws2.Cells[r2, 2].Value = p.NgayTao?.ToString("dd/MM/yyyy");
        ...
        public async Task<IActionResult> XacNhanChotSo(int month, int year)
        {
            ...
            if (role != RoleConstants.Accountant && role != RoleConstants.Director)
            {
            ...
            try
            {
                ...
                var phieus = await _context.PhieuDeXuats
                    .Where(p => p.NgayTao != null && p.NgayTao.Value.Month == month && p.NgayTao.Value.Year == year && p.TrangThai == "Đã duyệt")
                    .ToListAsync();
